...
class KanjiDetailModel(BaseModel):
    ...
    stroke_count: int = Field(..., description="The total number of strokes required to write the kanji.")
    meanings: List[str] = Field(..., description="A list of English meanings associated with the kanji.")
    ...
    unicode: str = Field(..., description="The Unicode codepoint for the kanji character (e.g., '871c').")
    heisig_en: Optional[str] = Field(
        None,
        description="The English keyword for the kanji according to James Heisig's 'Remembering the Kanji' method. "
                    "Null if not applicable."
    )
    freq_mainichi_shinbun: Optional[int] = Field( # New field
        None,
    ...
    unihan_cjk_compatibility_variant: Optional[str] = Field( # New field
        None,
...
class Variant(BaseModel):
    ...
    """
    written: str = Field(..., description="The written form of the variant (e.g., '食べる', '喰べる').")
    pronounced: Optional[str] = Field( # Marked Optional for robustness, though schema implies string
        None,
        description="The pronounced form of the variant, typically in kana (e.g., 'たべる')."
    )
