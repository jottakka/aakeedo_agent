...
async def test_fetch_kanji_details_success():
    ...
    assert isinstance(result, KanjiDetailModel), (
        f"Result for '{KANJI_FOR_DETAILS}' should be a KanjiDetailModel instance."
    )
    ...
    assert any("study" in m.lower() or "learn" in m.lower() for m in result.meanings), (
        "Meanings should relate to 'study' or 'learn'."
    )
...
async def test_fetch_joyo_kanji_list_success():
    ...
    assert all(isinstance(kanji, str) for kanji in result), (
        "All items in the Jōyō list should be strings."
    )
    assert all(len(kanji) == 1 for kanji in result if kanji), (
        "All kanji strings in the Jōyō list should be single characters."
    )
...
async def test_fetch_kanji_list_success_grade1():
    ...
    assert isinstance(result, list), (
        f"Result for kanji list '{KANJI_LIST_NAME_GRADE_1}' should be a list."
    )
    assert len(result) > 0, f"Kanji list '{KANJI_LIST_NAME_GRADE_1}' should not be empty."
    assert all(isinstance(kanji, str) for kanji in result), (
        f"All items in kanji list '{KANJI_LIST_NAME_GRADE_1}' should be strings."
    )
    assert all(len(kanji) == 1 for kanji in result if kanji), (
        f"All kanji in list '{KANJI_LIST_NAME_GRADE_1}' should be single characters."
    )
...
async def test_fetch_words_for_kanji_success_with_words():
    ...
    assert isinstance(result, list), (
        f"Result for words of kanji '{KANJI_FOR_WORDS}' should be a list."
    )
    assert len(result) > 0, f"Word list for '{KANJI_FOR_WORDS}' should not be empty."
    assert all(isinstance(entry, WordEntry) for entry in result), (
        f"All items for words of kanji '{KANJI_FOR_WORDS}' should be WordEntry instances."
    )
...
async def test_fetch_words_for_kanji_success_empty_or_few_words():
    ...
    assert isinstance(result, list), (
        f"Result for words of kanji '{KANJI_FOR_FEW_WORDS}' should be a list (can be empty)."
    )
    if result:  # If the list is not empty, check contents
        assert all(isinstance(entry, WordEntry) for entry in result), (
            f"All items for words of kanji '{KANJI_FOR_FEW_WORDS}' should be WordEntry instances."
        )
