...
)  # Assumes .env is in project root, tests in a subdir
...
@pytest.mark.skipif(
    not WANIKANI_API_TOKEN_FROM_ENV,
    reason="WANIKANI_API_TOKEN not found in environment variables or .env file.",
)
@pytest.mark.asyncio
async def test_fetch_wanikani_user_info_success_live():
    ...
    print(
        f"Attempting WaniKani API call with token: {'*' * (len(WANIKANI_API_TOKEN_FROM_ENV) - 4) + WANIKANI_API_TOKEN_FROM_ENV[-4:] if WANIKANI_API_TOKEN_FROM_ENV else 'None'}"
    )
